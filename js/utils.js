...
function fileurl(url,surl){
	if(this){
		const s=this;
		var xgurl=(((s.systems||{})[s.sys]||{}).url||s.c.img_site_url||s.xgimgurl||s.xgurl);
	}
	if(!isstr(url))return '';
...
function request(info) {
	...
	if(storage('session_id'))info.data['set_session_id']=storage('session_id');
	if(inxg()){
		var request=$.ajax;
	}else{
...
export {
	hook,hooks,reghook,copydata,goto,o,
	xgset,fileurl,inxg,download,openfile,filelist,unzip,popup,
	request,url,storage,loading,msg,confirm,getlocation,
	extend,obj2query,query2obj,replace,format,cachepath,md5,
	style4in1,px,ismobile,isemail,region,
	isstr,isobj,iseobj,isarr,isfun,isnum,isnan,isbool,isunde,isnull,isfile,type,obj2arr
};