<template>
	<view class="root" :class="classnames" :style="root">
		<checkbox-group v-if="multiple" ref="group" :class="['group','group-'+mode]" @change="change">
...
<script>
	...
	export default {
		...
		mounted() {
			...
			setTimeout(function(){s.check(v);},50);
		},
		methods: {
			change(e){
				const s=this;
...
.box .text{color:var(--theme-color-2,#000);}
...
.box.checked .text{color:var(--theme-color,--theme-color-2);font-weight:bold;}
</style>